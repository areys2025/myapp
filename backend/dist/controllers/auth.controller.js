...
const metamaskLogin = async (req, res) => {
    try {
        ...
        const user = await user_model_1.default.findOne({ walletAddress: address });
        if (!user) {
        ...
        if (token) {
            await (0, logEvent_1.logEvent)('User login', user === null || user === void 0 ? void 0 : user.email, user === null || user === void 0 ? void 0 : user.role, {
                userInfo: user._id,
                name: user.name
            });
        }
