...
const api = useApi();
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const allTasks = await api.getRepairTicketsForTech({ assignedTechnicianId: user.id });

      setTasks(allTasks.filter(task => task.status !== RepairStatus.COMPLETED && task.status !== RepairStatus.CANCELLED && task.status !== RepairStatus.PAID));
